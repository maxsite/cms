...
<?php
		...
		$form .= '<p>Если вы размещаете статьи сапы, то в файле <code>sape.php</code> (версия 1.3.9) в строке 1658 поменяйте строчку:</p><pre>
	protected $_data = array();  
	
	на..
	
	public $_data = array();</pre> <p>Это связано с изменением кода самой sape.</p>';
		
		
		
				
		$form .= '<p><label><input name="f_articles" type="checkbox"' . $checked_articles . '> Включить публикацию статей</label></p>
		<p>Не забудьте создать виджет «Sape.ru», где указать «Вывод статей».</p>';		
		...
		echo $form;
		echo '<button type="submit" name="f_submit" class="button i-save mar20-t">Сохранить изменения</button>';
		echo '</form>';
