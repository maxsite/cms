...
function perelinks_admin_page($args = [])
{
...
function perelinks_custom($content = '')
{
	...
	if ($k = mso_get_cache($cache_key)) {
		...
		if ($options['allowlate'] > 0) {
			...
			$CI->db->where('page_date_publish <', $pageData['page_date_publish']);
		}
	...
	$curr_page_slug = $pageData['page_slug']; // текущая страница - для ссылки
