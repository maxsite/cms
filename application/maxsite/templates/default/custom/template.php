...
function my_default_out_profiles($path = 'assets/css/profiles/')
{
	...
	if ($default_profiles = mso_get_option('default_profiles', getinfo('template'), array())) {
		// есть какие-то профили оформления
		$css_out = '';
...
function my_get_component_fn($option = '', $def_component = '')
{
	...
	if ($dir = mso_get_option($option, getinfo('template'), $def_component)) {
		...
		if (file_exists($fn)) return $fn;
			...
			$fn = getinfo('template_dir') . 'components/' . $dir . '/index.php';
			if (file_exists($fn)) return $fn;
		}
	}
