...
function mso_new_category($data)
{
    ...
    if ($ok) // все ок - выполняем sql запрос
    {
        ...
        } else {   // есть такая рубрика - не добавляем
...
function mso_edit_comuser($data)
{
    ...
    $data = mso_xss_clean_data($data, array('user_login', 'password', 'comusers_email', 'comusers_new_password', 'comusers_nik', 'comusers_icq', 'comusers_url', 'comusers_msn', 'comusers_jaber', 'comusers_skype', 'comusers_avatar_url', 'comusers_description', 'comusers_date_birth_y', 'comusers_date_birth_m', 'comusers_date_birth_d', 'comusers_notify', 'comusers_language', 'comusers_activate_key', 'comusers_activate_string', 'comusers_admin_note', 'comusers_date_registr'));
    ...
    $comusers_date_registr = isset($data['comusers_date_registr']) ? $data['comusers_date_registr'] : date('Y-m-d H:i:s');

    # проверка корректности даты регистрации
    if (!$comusers_date_registr) $comusers_date_registr = date('Y-m-d H:i:s');

    # исправляем автоматом на ближайший корректный вариант
    $comusers_date_registr = date("Y-m-d H:i:s", strtotime($comusers_date_registr));
    ...
    $upd_data = array(
        ...
        'comusers_date_registr' => $comusers_date_registr,
    );
