...
*/
function mso_get_page_cat($id = 0, $args = [])
{
	$id = (int) $id;
...
function mso_cat($li_format = '%NAME%', $checked_id = [], $type = 'page', $parent_id = 0, $order = 'category_menu_order', $asc = 'asc', $child_order = 'category_menu_order', $child_asc = 'asc')
{
...
function _get_child2($childs, $li_format = '', $checked_id = [], $list = '')
{
...
function mso_cat_array($type = 'page', $parent_id = 0, $order = 'category_menu_order', $asc = 'asc', $child_order = 'category_menu_order', $child_asc = 'asc', $in = false, $ex = false, $in_child = false, $hide_empty = false, $only_page_publish = false, $date_now = true, $get_pages = true)
{
	...
	if (($type != 'page') and ($type != 'links')) return [];
...
function mso_cat_array_single($type = 'page', $order = 'category_name', $asc = 'ASC', $type_page = '', $cache = true, $pages_detail = true, $page_status = 'publish')
{
	...
	foreach ($cats as $row) {
		...
		$r[$row['category_id']]['pages'] = [];
		$r[$row['category_id']]['links'] = [];
	...
	foreach ($r as $row) {
		...
		if ($parent > 0) {
			...
			while ($parent > 0) {
				// родитель родителя
				if (isset($r[$parent]['category_id_parent']))
					$parent = $r[$parent]['category_id_parent'];
				else
					$parent = 0;
...
function mso_cat_ul(
	...
	$checked_id = [], // номера, где меняет %CHECKED% на checked="checked" - для чекбоксов
	$selected_id = [], // номера, где отмечать <li class="selected">
	...
	$include = [], // только указанные рубрики и их дети
	$exclude = [] // исключить указанные рубрики
...
function _mso_cat_ul_glue($in, &$all, $li_format, $checked_id, $selected_id, $show_empty, $include, $exclude)
{
	...
	foreach ($in as $id) {
		...
		if ($include) {
			...
			// указанная рубрика и её дети
			if (!in_array($all[$id]['category_id'], $include) and !in_array($all[$id]['category_id_parent'], $include)) continue;
		}

		if ($exclude) {
			// исключить указанные рубрики
			if (in_array($all[$id]['category_id'], $exclude) or in_array($all[$id]['category_id_parent'], $exclude)) continue;
		}
		...
		if ($show_empty) {
			// если показывать все, то в любом случае выводим
			$out[] = $li . $add;
		} else {
...
function mso_get_cat_from_id($id = 0, $find_key = false)
{
	...
	if ($find_key === false) {
		// не указан ключ - вернуть все данные рубрики
