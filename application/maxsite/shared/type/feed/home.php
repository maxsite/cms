...
$par = ['limit' => $limit, 'cut' => $cut, 'type' => false, 'pagination' => false, 'only_feed' => true];
	...
	<rss version="2.0">
		<channel>
			...
			<?php foreach ($pages as $page) : extract($page); ?>
				<item>
					<title>
						<![CDATA[<?= xml_convert(strip_tags($page_title)) ?>]]>
					</title>
					...
					<description>
						<![CDATA[<?= mso_page_content($page_content) . mso_page_comments_link($page_comment_allow, $page_slug, ' ' . tf('Обсудить'), '', '', false) ?>]]>
					</description>
				</item>
			<?php endforeach; ?>
		</channel>
