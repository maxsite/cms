...
function editor_jw_autoload($args = array())
{
	mso_hook_add('editor_custom', 'editor_jw'); # хук на подключение своего редактора
}

function editor_jw_admin_header($args = '') 
{
	echo mso_load_style(getinfo('plugins_url') . 'editor_jw/jw/jquery.wysiwyg.css');
	mso_hook('editor_controls_extra_css');
}

function editor_jw($args = array()) 
{
	$editor_config['url'] = getinfo('plugins_url') . 'editor_jw/';
	$editor_config['dir'] = getinfo('plugins_dir') . 'editor_jw/';
