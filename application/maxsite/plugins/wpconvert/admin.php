	...
	if ( $post = mso_check_post(array('f_session_id', 'f_submit_chek', 'f_xml_file')) )
	{
		...
		else
		{
			...
			if (file_exists($fn))
			{
				...
				mso_flush_cache(false, 'rss'); // сброс кеша
			...
			if (file_exists($fn))
			{
				...
				if ($all)
				{
					...
					*/
					...
					echo $out . '<br><div class="update">' . t('Конвертирование выполнено!') . '</div>';
...
<p class="info"><?= t('С помощью данной страницы вы можете конвертировать WordPress-данные для MaxSite CMS. Для начала вам нужно выполнить экспорт из WordPress. Для выполнения экспорта загрузите файл <b>export-max.php</b> в каталог <b>wp-admin</b>. После этого наберите адрес <i>http://сайт/wp-admin/export-max.php</i> и выполните экспорт. Из-за некоторых ошибок и особенности формата, собственный wp-экспорт не подойдет для наших целей.') ?></p>

<p class="info"><?= t('Экспорт лучше сделать частями так, чтобы размер одного файла не превышал 300-400Кб. При конвертировании это позволит уменьшить нагрузку на сервер, а также позволит обойти ограничения хостинга на время выполнения скриптов и максимальный размер файла. В итоге у вас получится несколько xml-файлов.') ?></p>

<p class="info"><?= t('Перед конвертацией вам следует открыть каждый xml-файл в FireFox. Если браузер ругается на какие-то ошибки, то вам следует их исправить прямо в файле. К сожалению WordPress может неверно формировать xml-файл, но я постарался исправить ошибки в своем <b>export-max.php</b>') ?></p>

<p class="info"><?= t('Лишь только после того, как FireFox отобразит дерево элементов без ошибок, вы можете загрузить файл в каталог <b>uploads</b>. Можно через <b>Загрузки</b>.') ?></p>
...
<p class="info"><?= t('После конвертирования можно деактивировать этот плагин, а также удалить xml-файлы. Также рекомендую сбросить кэш.') ?></p>
...
<p class="info"><?= t('<u>ВАЖНО!</u> Настоятельно рекомедую перед началом конвертирования <u>сделать дамп текущей базы данных</u>! В случае ошибок, вы быстро сможете восстановить прежнее состояние своего сайта.') ?></p>
...
<?php
	...
	echo '<label><input type="checkbox" name="f_yes" nochecked> ' . t('Я понял и согласен взять на себя всю ответственность за использование данного конвертера') . '</label><br>';
	
	echo '<br>' . t('Выберите файл:') . ' <select style="width: 300px" name="f_xml_file">' . $option_files . '</select>';
	
	echo '<br><br><button type="submit" name="f_submit_chek" class="i check">' . t('Проверить файл') . '</button>';
	echo '<button type="submit" name="f_submit_go" class="i exchange">' . t('Запустить конвертацию') . '</button>';
