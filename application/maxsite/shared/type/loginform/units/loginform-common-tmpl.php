...
<div class="mso-loginform-common"><form method="post" action="{{ $action }}" name="flogin">
	...
	<p class="mso-loginform-login"><label><span>{{ $login }}</span><input type="text" value="" name="flogin_user" {{ $login_add }}></label></p>
	
	<p class="mso-loginform-password"><label><span>{{ $password }}</span><input type="password" value="" name="flogin_password" {{ $password_add }}></label></p>
