...
function forms_show_form($options, $files, $fields, $format)
{
	...
	elseif ($options['antispam'])
	{
		...
		$require_title = ' ' . $options['require_title'];
		$input = '<input class="mso-forms-antispam form-input" type="text" name="forms_fields[antispam]" required>';
	...
	$submit = '<button class="button" type="submit">' . tf('Отправить') . '</button>';
	
	if (!$options['reset']) 
		$reset = '';
	else
		$reset = '<button class="button" type="reset">' . tf('Очистить форму') . '</button>';
